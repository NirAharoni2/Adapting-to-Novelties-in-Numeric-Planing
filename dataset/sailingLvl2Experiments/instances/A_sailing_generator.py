def generate_sailing_problem(boat_positions, person_d_values, drift_factor, problem_name="sailing_problem", seed=1234):
    lines = []
    ...
    lines.append("")
    lines.append(f"\t(:init")
    lines.append(f"\t\t(= (drift_factor) {drift_factor})")
...
def write_sailing_instances_to_file(num_instances, output_dir, seed_start=1):
    ...
    for i in range(num_instances):
        ...
        drift_factor = round(random.uniform(2, 5),5)
...
def generate_sailing_problem2(boat_positions, person_d_values, problem_name="sailing_problem", seed=1234):
    lines = []
...
def write_sailing_instances_to_file2(num_instances, output_dir, seed_start=1):
    os.makedirs(output_dir, exist_ok=True)
    ...
    for i in range(num_instances):
        ...
        boat_positions, person_d_values = generate_random_sailing_instance()
        # Generate PDDL problem content
        content = generate_sailing_problem2(
            boat_positions,
            person_d_values,
            problem_name=problem_name,
            seed=problem_id
...
write_sailing_instances_to_file(
    num_instances=100,
    output_dir=r"C:\Felix\dataset\sailingNew\instances",
    seed_start=1
)
