...
def generate_instance(instance_name: str, min_factor_value, max_factor_value, num_of_factor,
    ...
    template_mapping = {
        ...
        "sled_supplies": "\n\t\t".join(
            [
                f"(= (sled_supplies s{i}) {random.uniform(0, sled_capacity_list[i])})"
                for i in range(num_of_sleds)
            ]
        ),
        "waypoint_supplies": "\n\t\t".join(
...
def generate_multiple_problems(
    ...
    for i in range(total_num_problems):
        ...
        min_sled_capacity = num_of_waypoints + 1
        max_sled_capacity = 2 * num_of_waypoints
...
def main():
    ...
    seed_id = 1
    random.seed(seed_id)
    file_path = Path(Config.get_database_dir() / "expeditionLvl2Experiments" / "instances" / f"seed_{seed_id}")
