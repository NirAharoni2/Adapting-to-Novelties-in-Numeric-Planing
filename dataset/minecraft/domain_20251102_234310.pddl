(define (domain polycraft)
    ...
    (:action get_log
     ...
     :precondition (and
            (>= (trees_in_map ) 1))
     :effect (and
            (assign (count_log_in_inventory ) (+ (* 0.99262 (count_log_in_inventory )) (+ (* 0.02683 (trees_in_map )) -0.26602)))
            (assign (trees_in_map ) (+ (* -0.00738 (count_log_in_inventory )) (+ (* 1.02683 (trees_in_map )) -0.26602))))
    )
    (:action craft_plank
     :parameters ()
     :precondition (and
