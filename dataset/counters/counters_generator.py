...
def generate_instance(instance_name: str, num_counters: int, max_int: int, num_rate_decrease: int) -> str:
    ...
    template_mapping = {
        ...
        "rate_decrease_list": " ".join([f"r{i}" for i in range(num_rate_decrease)]),
        ...
        "factor": f"(= (factor) {random.uniform(0.1, 1)})",
        "rate_decrease_initial_value": "\n\t".join(
            [
                f"(= (rate_value r{i}) {random.uniform(1, 4)})"
                for i in range(num_rate_decrease)
            ]
        ),
...
def generate_multiple_problems(
    min_counters: int,
    max_counters: int,
    num_rate_decrease: int,
    max_int: int,
    output_folder: Path,
    total_num_problems: int = 50,
) -> NoReturn:
    ...
    for i in range(total_num_problems):
        ...
        with open(output_folder / f"pfile{i + 1}.pddl", "wt") as problem_file:
            problem_file.write(
                generate_instance(
                    f"instance_{num_counters}_{max_int}", num_counters, max_int, num_rate_decrease
                )
            )
...
def main():
    generate_multiple_problems(
        min_counters=int(4),
        max_counters=int(6),
        num_rate_decrease=int(4),
        max_int=int(12),
        output_folder=Path("C:\Felix\dataset\counters\instances"),
    )
