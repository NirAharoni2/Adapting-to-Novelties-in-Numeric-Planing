...
def generate_pddl_instance_string2(max_x, max_y, max_z, battery_level, battery_level_full, problem_name, num_of_battery_factor):
    def location_name(x, y, z):
        return f"x{x}y{y}z{z}"
    ...
    lines.append(f"        (= (battery-level-full) {battery_level_full})")
    for i in range(num_of_battery_factor):
        lines.append(f"        (= (value bf{i}) {round(random.uniform(1, 1.4),5)})")
...
def write_grid_instances2(num_instances):
    ...
    for problem_id in range(start_id, start_id+num_instances):
        ...
        file_path = rf"C:\Felix\dataset\droneNew\instances\pfile{problem_id}.pddl"
        with open(file_path, "w") as f:
            f.write(content)
